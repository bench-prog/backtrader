...
class BusinessMetricsCollector(bt.Observer):
    ...
    def next(self):
        ...
        # 定期收集指标
        if (
            self.last_collect_time is None
            or current_time - self.last_collect_time >= self.p.collect_interval
        ):
    ...
    def _collect_business_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            metrics["completed_orders"] = len(
                [o for o in self._getorders() if not o.alive()]
            )
            ...
            metrics["daily_pnl_percent"] = (
                daily_pnl / self.broker.getvalue() if self.broker.getvalue() > 0 else 0
            )
...
class SystemResourceAlertRule(AlertRule):
    ...
    def __init__(
        self, resource_type: str, threshold: float, operator: str = ">", **kwargs
    ):
...
class BusinessAlertRule(AlertRule):
    ...
    def __init__(
        self, metric_name: str, threshold: float, comparison: str = ">", **kwargs
    ):
        super().__init__(
            f"business_{metric_name}_alert", f"业务{metric_name}告警", **kwargs
        )
...
# Backtrader Analyzer包装器
class RealTimeMonitoringAnalyzer(bt.Analyzer):
    """
    RealTimeMonitoringService的Analyzer包装器
    使其可以通过cerebro.addanalyzer()集成
    """

    def __init__(self):
        super(RealTimeMonitoringAnalyzer, self).__init__()
    ...
    def start(self):
        ...
        try:
            ...
            global_alert_mgr = get_alert_manager()
            # 复制通知渠道
            for channel in global_alert_mgr.notification_channels:
                self.monitoring_service.add_notification_channel(channel)
        except:
            pass
    ...
    def get_analysis(self):
        ...
        if self.monitoring_service:
            return {
                ...
                'alert_statistics': self.monitoring_service.get_alert_statistics(),
