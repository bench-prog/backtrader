...
class MeanVarianceOptimizer(bt.Analyzer):
    ...
    def optimize(self, data_names: List[str]) -> PortfolioWeights:
        ...
        try:
                ...
                constraints.append(
                    {
                        "type": "eq",
                        "fun": lambda x: (
                            np.dot(x, expected_returns) - self.p.target_return
                        ),
                    }
                )
            ...
            if result.success:
                ...
                if portfolio_risk > 0:
                    sharpe_ratio = (
                        portfolio_return - self.p.risk_free_rate
                    ) / portfolio_risk
                else:
                    sharpe_ratio = 0.0
                ...
                weights_dict = {
                    name: float(weight)
                    for name, weight in zip(data_names, optimal_weights)
                }
    ...
    def next(self):
        ...
        # 获取所有数据名称
        data_names = [
            data._name for data in self.strategy.datas if hasattr(data, "_name")
        ]
    ...
    def get_analysis(self):
        ...
        return {
            ...
            "current_portfolio": self.current_portfolio.__dict__
...
class RiskParityOptimizer(bt.Analyzer):
    ...
    def optimize(self, data_names: List[str]) -> PortfolioWeights:
        ...
        try:
            ...
            weights_dict = {
                name: float(weight) for name, weight in zip(data_names, weights)
            }
...
class PortfolioRebalancer:
    ...
    def rebalance(
        self, tolerance: float = 0.05, min_interval: int = 10
