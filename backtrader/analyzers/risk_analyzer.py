...
class RiskAnalyzer(bt.Analyzer):
    ...
    def _update_performance_metrics(self):
        ...
        try:
            ...
            if len(self.daily_returns) > 1:
                ...
                if volatility > 0:
                    sharpe_ratio = (
                        avg_return / volatility if len(self.daily_returns) > 1 else 0
                    )
    ...
    def _handle_new_day(self, current_date):
        ...
        try:
            ...
            # 简化处理：假设初始资金为10000
            daily_return = (
                (current_value - 10000) / 10000 if len(self.daily_returns) == 0 else 0
            )
    ...
    def _get_performance_summary(self):
        ...
        return {
            ...
            "critical_alerts": len(
                [a for a in self.risk_alerts if a.get("severity") == "CRITICAL"]
            ),
            "warning_alerts": len(
                [a for a in self.risk_alerts if a.get("severity") == "WARNING"]
            ),
...
class MarketStateAnalyzer(bt.Analyzer):
    ...
    def _analyze_volatility(self):
        ...
        if len(data) > 10:
            ...
            volatility = sum(
                abs(p - avg_price) / avg_price for p in recent_prices
            ) / len(recent_prices)
...
def create_risk_analyzers(
    max_drawdown=0.15, max_position_risk=0.3, daily_loss_limit=0.05
):
    """
    创建风险分析器集合的便捷函数
