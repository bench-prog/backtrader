...
class SystemHealthMonitor(bt.Analyzer):
    ...
    def _check_strategy_status(self, check_result: dict):
        ...
        try:
            # 检查是否有未完成订单
            pending_orders = len([order for order in self.strategy.broker.orders if not order.isbuy() and not order.issell()])
            
            # 检查持仓情况
            positions = sum(1 for data in self.strategy.datas if self.strategy.getposition(data).size != 0)
...
class PerformanceSummary(bt.Analyzer):
    ...
    def get_analysis(self):
        ...
        return {
            ...
            'return_pct': (self.final_value - self.initial_value) / self.initial_value if self.initial_value > 0 else 0
        }
