...
class PInfo(object):
    def __init__(self, sch):
        ...
        # Dynamic crosshair: maps ax -> list of (base_label, data_array)
        self.linedata = collections.defaultdict(list)
        # Maps ax -> legend object (for dynamic update)
        self.ax_legends = dict()
        # Maps legend text_obj -> (base_label, ...) for dynamic value update
        self.legend_text_data = dict()

    def newfig(self, figid, numfig, mpyplot):
        fig = mpyplot.figure(
            figid + numfig, facecolor=self.sch.facecolor, edgecolor=self.sch.edgecolor
        )
...
class Plot_OldSync(with_metaclass(MetaParams, object)):
    ...
    def __init__(self, **kwargs):
        ...
        if not hasattr(self.p.scheme, "locbg"):
            setattr(self.p.scheme, "locbg", self.p.scheme.facecolor)
            setattr(self.p.scheme, "locbgother", self.p.scheme.facecolor)
    ...
    def _bind_legend_to_data(self, legend, *axes):
        """Match legend text entries to stored linedata by text prefix."""
        all_entries = []
        for a in axes:
            all_entries.extend(self.pinf.linedata.get(a, []))
        if not all_entries:
            return
        for text_obj in legend.get_texts():
            ...
            for entry in all_entries:
                ...
                if base and txt.startswith(base):
                    self.pinf.legend_text_data[text_obj] = entry
                    break

    def _on_mouse_move(self, event):
        """Update legend values dynamically based on mouse x-position."""
        if event.inaxes is None:
            return
        if not self.pinf.legend_text_data:
            return

        idx = int(round(event.xdata))
        if idx < 0 or idx >= self.pinf.xlen:
            return

        changed = False
        for text_obj, entry in self.pinf.legend_text_data.items():
            base = entry[0]
            try:
                if len(entry) >= 3 and entry[1] == "ohlc":
                    _, _, o, h, l, c = entry
                    if idx < len(o):
                        new = "%s O:%.2f H:%.2f L:%.2f C:%.2f" % (
                            base,
                            o[idx],
                            h[idx],
                            l[idx],
                        ...
                        continue
                elif len(entry) >= 3 and entry[1] == "linec":
                    _, _, c = entry
                    if idx < len(c):
                        new = "%s C:%.2f" % (base, c[idx])
                    else:
                        continue
                elif len(entry) == 2:
                    _, data_arr = entry
            ...
            if text_obj.get_text() != new:
                text_obj.set_text(new)
                changed = True
    ...
    def plot(
        ...
        for numfig in range(numfigs):
            ...
            cursor = MultiCursor(
                ...
                horizShared=True,
                vertShared=False,
                color=self.pinf.sch.cursor_color,
                lw=self.pinf.sch.cursor_width,
                ls=self.pinf.sch.cursor_style,
            )

            self.pinf.cursors.append(cursor)

            # Register dynamic legend update callback
    ...
    def newaxis(self, obj, rowspan):
        ...
        ax.grid(
            ...
            which="both",
            color=sch.gridcolor,
            linestyle=sch.gridstyle,
            linewidth=sch.gridwidth,
            alpha=sch.gridalpha,
        )

        # Spine styling
        ax.set_facecolor(sch.facecolor)
        for spine_name, visible in [
            ("left", sch.spines_left),
            ("top", sch.spines_top),
            ("bottom", sch.spines_bottom),
            ("right", sch.spines_right),
        ]:
            ax.spines[spine_name].set_visible(visible)
            if visible:
                ax.spines[spine_name].set_color(sch.spine_color)
    ...
    def plotind(
        ...
        for lineidx in range(ind.size()):
            ...
            # Build the base label (without value) for dynamic crosshair
            _base_label = label
            ...
            # Store line data for dynamic legend updates
            if label and not label.startswith("_nolegend"):
                self.pinf.linedata[ax].append((_base_label, list(lplot)))
            ...
            if self.pinf.sch.legendind and ind.plotinfo._get("plotlegend", True):
                ...
                if labels:
                    ...
                    legend = ax.legend(
                        loc=loc,
                        numpoints=1,
                        frameon=sch.legend_frameon,
                        framealpha=sch.legend_framealpha,
                        shadow=sch.legend_shadow,
                        fancybox=sch.legend_fancybox,
                        prop=self.pinf.prop,
                    )

                    # Store legend reference for dynamic updates
                    self.pinf.ax_legends[ax] = legend

                    # Bind legend texts to data for dynamic crosshair
                    self._bind_legend_to_data(legend, ax)
            ...
            if voloverlay:
                ...
                if handles:
                    ...
                    legend = ax.legend(
                        loc=loc,
                        numpoints=1,
                        frameon=self.pinf.sch.legend_frameon,
                        framealpha=self.pinf.sch.legend_framealpha,
                        shadow=self.pinf.sch.legend_shadow,
                        fancybox=self.pinf.sch.legend_fancybox,
                        prop=self.pinf.prop,
                    )
    ...
    def plotdata(self, data, indicators):
        ...
        _base_datalabel = datalabel  # save before value is appended
        ...
        # Store data for dynamic crosshair legend updates
        if self.pinf.sch.style.startswith("line"):
            self.pinf.linedata[ax].insert(0, (_base_datalabel, "linec", list(closes)))
        else:
            self.pinf.linedata[ax].insert(
                0,
                (
                    _base_datalabel,
                    "ohlc",
        ...
        if handles:
            ...
            # Bind legend texts to data for dynamic crosshair
            self._bind_legend_to_data(legend, ax, a)
            downinds = self.dplotsdown[ind]
            for downind in downinds:
                self.plotind(
                    data,
