...
class ConfigValidator:
    ...
    def add_validator(self, section: str, field: str, validator_func: callable, required: bool = False):
        """添加字段验证器"""
    ...
    def register_common_validators(self):
        """注册常用验证器"""
        # 数值验证
        self.add_validator('risk', 'max_leverage', lambda x: x > 0 and x <= 10, required=True)
        self.add_validator('risk', 'max_position_size', lambda x: x > 0 and x <= 1, required=True)
        
        # 字符串验证
        self.add_validator('database', 'host', lambda x: isinstance(x, str) and len(x) > 0, required=True)
        self.add_validator('database', 'port', lambda x: isinstance(x, int) and 1 <= x <= 65535, required=True)
        
        # 布尔验证
        self.add_validator('logging', 'enabled', lambda x: isinstance(x, bool), required=True)
...
class ConfigVersionManager:
    ...
    def rollback_to_version(self, version_hash: str) -> Dict:
        ...
        with self._lock:
            ...
            new_version = self.create_version(target_config, f"回滚到版本 {version_hash}")
...
class ConfigurationManager:
    ...
    def set_config(self, config: Dict, description: str = ""):
        """设置配置"""
        with self._config_lock:
            ...
            # 创建新版本
            version_hash = self.version_manager.create_version(config, description)
            
            # 更新当前配置
            self.current_config = copy.deepcopy(config)
            
            # 通知监听器
            self._notify_watchers('config_updated', {
    ...
    def update_config(self, section: str, key: str, value: Any, description: str = ""):
        ...
        with self._config_lock:
            ...
            version_hash = self.version_manager.create_version(
                self.current_config, 
                f"更新 {section}.{key}: {description}"
            )
            
            # 通知监听器
            self._notify_watchers('config_item_updated', {
                'section': section,
                'key': key,
                'value': value,
                'version': version_hash
...
def load_config_from_environment(config_manager: ConfigurationManager, 
                               env_prefix: str = "BT_") -> None:
    """从环境变量加载配置"""
    config_updates = {}
    
    # 系统配置
    if f"{env_prefix}TIMEZONE" in os.environ:
        config_updates.setdefault('system', {})['timezone'] = os.environ[f"{env_prefix}TIMEZONE"]
    
    # 风控配置
    if f"{env_prefix}MAX_LEVERAGE" in os.environ:
        config_updates.setdefault('risk', {})['max_leverage'] = float(os.environ[f"{env_prefix}MAX_LEVERAGE"])
    
    if f"{env_prefix}MAX_POSITION_SIZE" in os.environ:
        config_updates.setdefault('risk', {})['max_position_size'] = float(os.environ[f"{env_prefix}MAX_POSITION_SIZE"])
    
    # 日志配置
    if f"{env_prefix}LOG_LEVEL" in os.environ:
        config_updates.setdefault('logging', {})['level'] = os.environ[f"{env_prefix}LOG_LEVEL"]
