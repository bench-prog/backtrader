...
class RiskBasedSizer(bt.Sizer):
    ...
    def _calculate_atr(self, data, period):
        ...
        for i in range(period):
            ...
            high = data.high[-(i+1)]
            low = data.low[-(i+1)]
            prev_close = data.close[-(i+2)] if i+2 <= len(data) else data.close[-(i+1)]
...
# 便捷函数
def create_risk_sizer(risk_percent=0.02, max_position=0.3, volatility_period=20, 
                     atr_multiplier=2.0, sizer_type='atr'):
    """
    创建风险Sizer的便捷函数
