...
class PositionConcentrationRule(RiskRule):
    ...
    def check(self, context: Dict[str, Any]) -> tuple:
        ...
        if concentration > self.max_concentration:
            self.violation_count += 1
            self.last_violation = datetime.datetime.now()
            return False, f"持仓集中度过高: {concentration:.2%} > {self.max_concentration:.2%}"
        return True, ""
...
class RiskConfigurationManager:
    ...
    def __init__(self, account_id: str = "default"):
        ...
        self.rule_groups['real_time'].extend([
            'daily_loss_limit'
        ])
    ...
    def check_rules(self, context: Dict[str, Any], group: str = None) -> Dict[str, tuple]:
        """
        执行规则检查
    ...
    def _trigger_violation_callbacks(self, rule_name: str, message: str, context: Dict[str, Any]):
        """触发违规回调"""
        violation_info = {
    ...
    def _create_rule_from_config(self, name: str, config: Dict) -> RiskRule:
        ...
        rule_mapping = {
            ...
            'PositionConcentrationRule': lambda: PositionConcentrationRule(enabled=enabled, **params),
...
class MultiAccountRiskManager:
    ...
    def check_all_accounts(self, context_getter: Callable[[str], Dict]) -> Dict[str, Dict]:
        """检查所有账户"""
        with self._lock:
            results = {}
...
# 便捷函数
def load_risk_config_from_file(filepath: str, account_id: str = "default") -> RiskConfigurationManager:
    """从文件加载风控配置"""
    manager = RiskConfigurationManager(account_id)
...
def save_risk_config_to_file(manager: RiskConfigurationManager, filepath: str, format: str = 'json'):
    """保存风控配置到文件"""
