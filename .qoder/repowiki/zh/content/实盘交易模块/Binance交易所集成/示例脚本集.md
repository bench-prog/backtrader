...
- [real_trade/examples/query_orders.py](file://real_trade/examples/query_orders.py)
...
## 更新摘要
**所做更改**
- 更新示例文件位置：从 `real_trade/binance/examples/` 迁移到 `real_trade/examples/`
- 更新导入路径：修改 sys.path.insert 路径从 `../..` 到 `../..`
- 更新配置文件引用：保持原有的配置文件结构和使用方式
- 更新示例脚本的相对路径引用
...
**更新** 本版本重点介绍了新的统一配置方法，通过 GlobalConfig 类和 create_binance_engine_from_config 函数简化了配置加载和策略参数传递过程。示例文件已从 `real_trade/binance/examples/` 迁移到 `real_trade/examples/`，导入路径已相应更新。

## 项目结构
示例脚本位于 `real_trade/examples/` 目录下，按用途分为基础示例、API连接测试、交易功能测试等模块。examples 目录包含配置示例、Futures Testnet测试、可用交易对检查、订单查询等功能脚本，新的统一配置系统通过 GlobalConfig 提供灵活的配置加载能力。
...
EX4["query_orders.py<br/>订单查询"]
EX5["live_trading.py<br/>实盘交易示例"]
EX6["quickstart.py<br/>快速上手示例"]
end
subgraph "测试脚本"
...
CFG3["config/binance/README.md<br/>配置使用说明"]
...
- [real_trade/tools/binance/config_generator.py](file://real_trade/tools/binance/config_generator.py#L1-L105)
- [real_trade/tools/binance/config_validator.py](file://real_trade/tools/binance/config_validator.py#L1-L185)

**章节来源**
- [real_trade/examples/config_example.py](file://real_trade/examples/config_example.py#L1-L259)
- [real_trade/utils/config.py](file://real_trade/utils/config.py#L1-L169)
- [real_trade/config/binance/README.md](file://real_trade/config/binance/README.md#L1-L132)
...
**章节来源**
- [real_trade/utils/config.py](file://real_trade/utils/config.py#L1-L169)
- [real_trade/__init__.py](file://real_trade/__init__.py#L137-L223)
- [real_trade/tools/binance/config_generator.py](file://real_trade/tools/binance/config_generator.py#L1-L105)
- [real_trade/tools/binance/config_validator.py](file://real_trade/tools/binance/config_validator.py#L1-L185)
...
**章节来源**
- [real_trade/examples/query_orders.py](file://real_trade/examples/query_orders.py#L1-L132)

### 实盘交易示例：live_trading.py
- **目标**：演示完整的实盘交易流程：引擎 + 风控 + 监控 + 通知。
- **关键组件**：
  - TradingRunner：统一的交易运行器
  - RiskManager：风控管理器
...
### 快速上手：quickstart.py
- **目标**：演示如何用最少代码启动一个策略回测。
- **两种启动方式**：
  - 使用 TradingRunner（推荐）：一行代码启动完整流程
  - 手动组装：传统方式创建 Store/Broker/Data
- **参数配置**：
  - exchange、symbol、timeframe
  - strategy_cls、strategy_params
  - testnet、paper_trading、backtest
  - historical_limit 等
...
- **功能特性**：
  ...
  - 支持扁平和嵌套两种 JSON 结构。
- **使用方法**：
  - 运行 python tools/binance/config_generator.py
  - 按提示输入各项配置参数
...
- **示例/测试脚本依赖**：
  - 所有脚本通过 sys.path.insert 引入 real_trade 接口。
  - 新的统一配置方法简化了配置加载过程。
...
IF --> EX["examples/*.py"]
IF --> TS["tests/*.py"]
CG["tools/binance/config_generator.py"] --> GC
CV["tools/binance/config_validator.py"] --> GC
...
**章节来源**
- [real_trade/utils/config.py](file://real_trade/utils/config.py#L1-L169)
- [real_trade/__init__.py](file://real_trade/__init__.py#L1-L223)
...
**章节来源**
- [real_trade/examples/testnet_futures_test.py](file://real_trade/examples/testnet_futures_test.py#L142-L151)
- [real_trade/examples/check_available_symbols.py](file://real_trade/examples/check_available_symbols.py#L150-L159)
- [real_trade/examples/query_orders.py](file://real_trade/examples/query_orders.py#L82-L98)
- [real_trade/tools/binance/config_validator.py](file://real_trade/tools/binance/config_validator.py#L81-L175)

## 结论
示例脚本集提供了从统一配置管理、连接测试、订单验证到交易测试与统计分析的完整链路。新的 GlobalConfig 统一配置系统大幅简化了配置加载和策略参数传递过程，通过 create_binance_engine_from_config 函数实现了灵活的配置管理。示例文件已迁移至 `real_trade/examples/` 目录，导入路径已相应更新。通过回测与 Demo 实盘结合，既能快速验证策略逻辑，又能逐步过渡到真实市场。建议按顺序运行：先用 config_example.py 熟悉新的统一配置方法，再用 testnet_futures_test.py 验证连接，随后用各类订单测试脚本验证交易流程，最后用连续交易测试评估系统稳定性。
...
- **配置生成器**：
  ...
  - 生成标准格式的 JSON 配置文件
- **配置验证器**：
  - 运行 python tools/binance/config_validator.py my_config.json
  - 获取详细的验证结果和建议
  - 修复配置中的错误和警告
...
- **示例文件位置**：示例文件现已迁移到 `real_trade/examples/` 目录，导入路径已自动更新。