...
class SampleTester:
    def __init__(self, samples_dir='samples', python_exe=None, timeout=30, verbose=False):
    ...
    def run_all(self):
        ...
        print(f"\n{'#'*70}")
        print(f"# Backtrader Sample Test Runner")
    ...
    def print_summary(self):
        ...
        print(f"{'='*70}")
        print(f"Total:   {total}")
        print(f"✅ Passed:  {len(self.results['passed'])} ({len(self.results['passed'])/total*100:.1f}%)")
        print(f"❌ Failed:  {len(self.results['failed'])} ({len(self.results['failed'])/total*100:.1f}%)")
        print(f"⏱️  Timeout: {len(self.results['timeout'])} ({len(self.results['timeout'])/total*100:.1f}%)")
        print(f"⚠️  Skipped: {len(self.results['skipped'])} ({len(self.results['skipped'])/total*100:.1f}%)")
        print(f"{'='*70}")
...
def main():
    ...
    parser.add_argument('--pattern',
                        help='Only test samples matching this pattern (e.g., "data-*")')
    ...
    if args.pattern:
        ...
        print(f"Filtered {len(all_samples)} samples to {len(filtered)} matching '{args.pattern}'")
