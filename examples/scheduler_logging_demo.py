...
from backtrader.logging import setup_trading_logger, LoggingStrategyMixin
...
class ScheduledTradingStrategy(LoggingStrategyMixin, bt.Strategy):
    ...
    def stop(self):
        """策略结束"""
        self.log(f'(SMA {self.params.sma_period}) 最终价值: {self.broker.getvalue():.2f}')
...
def demonstrate_scheduler_and_logging():
    ...
    # 2.2 启用日志增强
    logger = setup_trading_logger(log_dir="demo_logs", retention_days=7)
    print("✓ 日志增强已启用")
    ...
    job_id = schedule_daily_strategy(
        ...
        hour=9, 
        minute=30, 
        strategy_name="morning_strategy"
    )
    ...
    log_risk_event("position_check", "FAIL", {"symbol": "BTC/USDT", "size": 0.5, "limit": 0.3})
...
def demonstrate_minimal_integration():
    ...
    from backtrader.logging import setup_trading_logger
    
    scheduler = enable_scheduler(cerebro)  # 启用调度
    logger = setup_trading_logger()        # 启用日志
