...
class GridTradingStrategy(bt.Strategy):
    ...
    def _initialize_grids(self):
        ...
        if self.p.printlog:
            ...
            self.log(
                f"价格范围: ${self.grid_prices[0]:.2f} - ${self.grid_prices[-1]:.2f}"
            )
    ...
    def next(self):
        ...
        self.max_drawdown = max(
            self.max_drawdown, (self.peak_value - current_value) / self.peak_value
        )
            ...
            if order.status in [
                ...
                if order.status == order.Completed:
                    ...
                    if order.isbuy():
                        ...
                        self.log(
                            f"买单成交: 价格${order.executed.price:.2f}, 数量{order.executed.size}"
                        )
                    else:
                        # 计算盈亏
                        entry_price = self.position_prices.get(
                            order_ref, order.executed.price
                        )
                        ...
                        self.log(
                            f"卖单成交: 价格${order.executed.price:.2f}, 盈亏${pnl:.2f}"
                        )
    ...
    def _update_grids(self):
        ...
        if self.p.printlog:
            self.log(
                f"网格更新完成，新价格范围: ${self.grid_prices[0]:.2f} - ${self.grid_prices[-1]:.2f}"
            )
    ...
    def _trigger_grid_order(self, grid_price):
        ...
        if grid_key in [
            order_info.get("grid_key") for order_info in self.active_orders.values()
        ]:
            return
        ...
        if order:
            ...
            self.log(
                f"网格订单触发: {order_type} {position_size}股 @ ${grid_price:.2f}"
            )
...
def run_grid_trading_demo():
    ...
    if hasattr(trades_analysis, "won") and trades_analysis.won.total > 0:
        ...
        print(
            f"  胜率: {trades_analysis.won.total / trades_analysis.total.total * 100:.1f}%"
        )
