...
def demonstrate_environment_management():
    ...
    for env in environments:
        ...
        try:
            ...
            print(
                f"最大杠杆: {current_config.get('risk', {}).get('max_leverage', 'N/A')}"
            )
            print(
                f"最大仓位: {current_config.get('risk', {}).get('max_position_size', 'N/A')}"
            )
            print(
                f"调试模式: {current_config.get('environment', {}).get('debug', 'N/A')}"
            )
...
def demonstrate_secret_management():
    ...
    for log_entry in access_log:
        print(
            f"  {log_entry['timestamp']} - {log_entry['action']} - {log_entry['key']}"
        )
...
def demonstrate_config_validation():
    ...
    print("🧪 测试有效配置:")
    valid_config = {
        "risk": {"max_leverage": 2.0, "max_position_size": 0.1, "max_daily_loss": 0.05},
        "database": {"host": "localhost", "port": 5432, "database": "backtrader"},
...
def demonstrate_version_control():
    ...
    if len(versions) >= 2:
        ...
        diff = config_mgr.version_manager.compare_versions(
...
def demonstrate_project_initialization():
    ...
    try:
        ...
        config_mgr = bt.config.initialize_environment(
            env_name="development", project_path=temp_dir
        )
...
def demonstrate_security_features():
    ...
    if access_log:
        ...
        print(
            f"  最后访问: {last_access['timestamp']} - {last_access['action']} - {last_access['key']}"
        )
